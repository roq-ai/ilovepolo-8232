...
import { getShirtDesigns } from 'apiSdk/shirt-designs';
import { ShirtDesignInterface } from 'interfaces/shirt-design';

import { ShirtDesignListPage as ShowCaseList } from 'pages/shirt-designs';

function DashboardPage() {
  const { session } = useSession();

  const shirtDesignStat = useSWR<PaginatedInterface<ShirtDesignInterface>>(
    () => `/shirt-designs`,
    () => getShirtDesigns({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    {
      title: 'Shirt Designs',
      result: shirtDesignStat?.data?.totalCount,
      isLoading: shirtDesignStat.isLoading,
    },
  ];
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`shirt-designs`} passHref legacyBehavior>
                  <Button
